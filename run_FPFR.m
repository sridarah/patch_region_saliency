%% This code demonstrates the faster variant of the proposed salient region detection algorithm "FPFR"
...
for n = 1:no_of_images
    if strcmp(input_dir(n).name(end),'g') || strcmp(input_dir(n).name(end),'p')
        ...
        imwrite(salMap, ['Output\',input_dir(n).name(1:end-4),'_FPFR.png']);  % Writig the output saliency maps
